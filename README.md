...
현재 배포된 테스트셋의 경우 국회도서관 국가학술정보 클라우드의 제22대(~2024년) 국회 소위원회 회의록 데이터셋을 수정하여 할루시네이션이 삽입된 요약문들이 들어있는 데이터입니다.

현재 아직 연구 진행중이라 intrinsic 분류에는 과탐 및 확신의 경향이 존재합니다.
...
## 개선사항
 ...
 현재 데이터셋에서 같은 라벨의 경우 특정 단어가 들어가있는 경우가 많다.
 즉, 모델이 할루시네이션의 특징이 아니라 그 단어가 들어가면 무조건 intrinsic 할루시네이션이라 잡는 shortcut학습을 하여 오탐이 높아졌다.
 그래서 그런 단어들이 들어갔지만 정상요약문인 데이터들을 train set으로 추가하여 shortcut 학습으로 인한 오탐을 낮추는게 현 상황에서의 목표이다.
